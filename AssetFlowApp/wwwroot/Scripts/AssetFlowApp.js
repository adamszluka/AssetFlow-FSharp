...
function Main(){
  ...
  const updateAssetStatus=(assetId, newStatus) => assetsVar.Set(map((asset) => asset.Id===assetId?New(asset.Id, asset.Name, asset.Owner, asset.AssetType, newStatus, asset.PurchaseYear):asset, assetsVar.Get()));
  ...
  const statusFilterButton=(label, value) => Doc.BindView((currentFilter) => Doc.Element("button", [Attr.HandlerImpl("click", () =>() => statusFilterVar.Set(value)), Attr.Create("style", Equals(currentFilter, value)?"margin-right: 8px; margin-bottom: 8px; padding: 8px 12px; border-radius: 8px; border: none; background: #37474f; color: white; font-weight: bold; cursor: pointer;":"margin-right: 8px; margin-bottom: 8px; padding: 8px 12px; border-radius: 8px; border: 1px solid #cccccc; background: white; cursor: pointer;")], [Doc.TextNode(label)]), statusFilterVar.View);
...
function replacementColor(asset){
  return isReplacementDue(asset)?"#c62828":"#2e7d32";
}
...
function filterByStatus(statusFilter, assets){
  return statusFilter.$==1?filter((asset) => asset.Status.$===0, assets):statusFilter.$==2?filter((asset) => asset.Status.$===1, assets):statusFilter.$==3?filter((asset) => asset.Status.$===2, assets):statusFilter.$==4?filter((asset) => asset.Status.$===3, assets):assets;
}
...
let OnlyDesktops={$:2};
let OnlyServers={$:3};
let OnlyVirtualMachines={$:4};
let OnlyNetworkDevices={$:5};
let OnlyOther={$:6};
...
function New(Id, Name, Owner, AssetType, Status, PurchaseYear){
  return{
    Id:Id, 
    Name:Name, 
    Owner:Owner, 
